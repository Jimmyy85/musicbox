...
const TRANSLATIONS: any = {
    fr: {
        discover: "Découverte", search: "Recherche", activity: "Activité", profile: "Profil",
    ...
    en: {
        discover: "Discover", search: "Search", activity: "Activity", profile: "Profile",
...
export default function Home() {
    ...
    const addToList = async (listId: string) => {
        ...
        await supabase.from('list_items').insert([{
            list_id: listId, item_key: getKey(selectedItem), title: getTitle(selectedItem), artist: getArtist(selectedItem), image: getCover(selectedItem), type: getType(selectedItem)
        }]);
    ...
    useEffect(() => { if (profile?.username) supabase.from('follows').select('*', { count: 'exact', head: true }).eq('following_username', profile.username).then(({ count }) => setFollowersCount(count || 0)); }, [profile]);
    ...
    return (
        <div style={{ backgroundColor: COLORS.bg, minHeight: '100vh', color: COLORS.textMuted, fontFamily: 'Inter, sans-serif' }}>
            ...
            {/* NOTIFICATIONS PANEL */}
            {showNotifs && (
                <div style={{ position: 'fixed', top: '60px', right: '10px', width: '250px', background: '#111', border: `1px solid ${COLORS.border}`, borderRadius: '10px', zIndex: 9999, padding: '10px' }}>
            ...
            <main style={{ maxWidth: '1000px', margin: '0 auto', padding: '80px 20px 120px' }}>
                ...
                {activeTab === 'discover' && (
                    <div>
                        ...
                        {dailyDrop && (
                            <div onClick={() => setSelectedItem(dailyDrop)} style={{ marginBottom: '30px', position: 'relative', height: '180px', borderRadius: '20px', overflow: 'hidden', cursor: 'pointer', border: `1px solid ${COLORS.border}` }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '10px', marginBottom: '30px' }}>
                            {(viewedProfile ? viewedTop4 : myTop4).concat(Array(4).fill(null)).slice(0, 4).map((a, i) => (
                                <div key={i} style={{ aspectRatio: '1/1', background: 'rgba(255,255,255,0.05)', borderRadius: '10px', border: `1px dashed ${COLORS.border}`, overflow: 'hidden' }}>
                                    {a && <img src={a.image} onClick={() => setSelectedItem(a)} style={{ width: '100%', height: '100%', objectFit: 'cover', cursor: 'pointer' }} />}
                                </div>
                            ))}
                        </div>
                                ...
                                <div style={{ display: 'flex', alignItems: 'flex-end', height: '40px', gap: '5px' }}>
                                    {userStats.distribution.map((h, i) => (
                                        <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', height: '100%', justifySelf: 'flex-end' }}>
                                            <div style={{ width: '8px', height: `${h || 1}%`, background: COLORS.accent, borderRadius: '2px 2px 0 0', marginTop: 'auto' }}></div>
                                        </div>
                                    ))}
                                </div>
                            </div>
                        ...
                        {query.length < 3 && (
                            <div style={{ marginTop: '30px' }}>
                                ...
                                {recentSearches.length > 0 && (
                                    <>
                                        <h4 style={{ color: COLORS.textMuted, marginBottom: '15px', marginTop: '30px' }}>{t('recent')}</h4>
                                        {recentSearches.map((s, i) => (
                                            <div key={i} onClick={() => setQuery(s)} style={{ padding: '15px 0', borderBottom: `1px solid ${COLORS.border}`, color: 'white', cursor: 'pointer' }}>🕒 {s}</div>
                                        ))}
                                    </>
                                )}
                            </div>
            ...
            {/* SETTINGS MODAL */}
            {showSettings && (
                <div style={{ position: 'fixed', inset: 0, zIndex: 6000, background: 'rgba(0,0,0,0.95)', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                    <div style={{ width: '90%', maxWidth: '400px', background: '#111', padding: '30px', borderRadius: '20px', border: `1px solid ${COLORS.border}` }}>
                        <h2 style={{ color: 'white', marginBottom: '20px' }}>{t('settings')}</h2>
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: '20px' }}>
                            <img src={tempAvatar} style={{ width: '80px', height: '80px', borderRadius: '50%', marginBottom: '10px', objectFit: 'cover' }} />
                            <input type="file" accept="image/*" ref={fileInputRef} onChange={handleImageUpload} style={{ display: 'none' }} />
                            <button onClick={() => fileInputRef.current?.click()} style={{ fontSize: '12px', padding: '5px 15px', background: COLORS.surface, color: 'white', border: `1px solid ${COLORS.border}`, borderRadius: '10px' }}>{t('change_photo')}</button>
                        </div>
                        <input value={tempPseudo} onChange={e => setTempPseudo(e.target.value)} style={{ width: '100%', padding: '10px', background: 'black', border: `1px solid ${COLORS.border}`, color: 'white', borderRadius: '8px', marginBottom: '15px' }} />
                        <textarea value={tempBio} onChange={e => setTempBio(e.target.value)} placeholder={t('bio_placeholder')} style={{ width: '100%', height: '80px', padding: '10px', background: 'black', border: `1px solid ${COLORS.border}`, color: 'white', borderRadius: '8px', marginBottom: '20px' }} />
                        <div style={{ display: 'flex', gap: '10px' }}><button onClick={() => setShowSettings(false)} style={{ flex: 1, padding: '10px', background: 'none', border: `1px solid ${COLORS.border}`, color: 'white', borderRadius: '10px' }}>Annuler</button><button onClick={saveSettings} style={{ flex: 1, padding: '10px', background: COLORS.accent, border: 'none', color: 'white', borderRadius: '10px', fontWeight: 'bold' }}>{loading ? "..." : t('save')}</button></div>
                    </div>
                </div>
            )}
            ...
            {/* ANTHEM SEARCH MODAL */}
            {showAnthemSearch && (
                <div style={{ position: 'fixed', inset: 0, zIndex: 5000, background: 'rgba(0,0,0,0.9)', padding: '20px' }}>
                    <button onClick={() => setShowAnthemSearch(false)} style={{ color: 'white', background: 'none', border: 'none', fontSize: '20px', marginBottom: '20px' }}>✕ Fermer</button>
                    <h2 style={{ color: 'white' }}>{t('choose_anthem')}</h2>
                    <input value={anthemQuery} onChange={e => setAnthemQuery(e.target.value)} placeholder={t('search_placeholder')} style={{ width: '100%', padding: '15px', borderRadius: '10px', border: 'none' }} autoFocus />
                    <div style={{ marginTop: '20px' }}>{anthemResults.map(t => (<div key={t.id} onClick={() => setAnthem(t)} style={{ display: 'flex', gap: '15px', padding: '10px', borderBottom: `1px solid ${COLORS.border}`, cursor: 'pointer' }}><img src={t.album.cover_small} style={{ borderRadius: '5px' }} /><div><div style={{ color: 'white', fontWeight: 'bold' }}>{t.title}</div><div style={{ color: COLORS.textMuted, fontSize: '12px' }}>{t.artist.name}</div></div></div>))}</div>
                </div>
            )}
                ...
                <div style={{ position: 'fixed', inset: 0, zIndex: 200, background: '#0a0c0f', overflowY: 'auto' }}>
                    ...
                    <div style={{ padding: '20px', maxWidth: '600px', margin: '0 auto' }}>
                        ...
                        <div style={{ textAlign: 'center' }}>
                            ...
                            {getType(selectedItem) === 'track' && selectedItem.preview && (
                                <button onClick={() => playPreview(selectedItem.preview)} style={{ background: COLORS.track, border: 'none', color: 'white', padding: '10px 30px', borderRadius: '30px', fontWeight: 'bold', margin: '15px 0' }}>{playingUri === selectedItem.preview ? t('pause') : t('listen')}</button>
                            )}
                        ...
                        <div>{communityComments.filter(c => !c.parent_id).map((c, i) => (
                            <div key={i} style={{ marginBottom: '15px', paddingBottom: '15px', borderBottom: `1px solid ${COLORS.border}` }}>
                                ...
                                <div style={{ display: 'flex', gap: '10px' }}><button onClick={() => handleLike(c.id, c.likes)} style={{ background: 'none', border: 'none', color: 'white', fontSize: '12px' }}>❤ {c.likes || 0}</button><button onClick={() => setReplyTo(replyTo === c.id ? null : c.id)} style={{ background: 'none', border: 'none', color: COLORS.textMuted, fontSize: '12px' }}>{t('reply')}</button></div>
